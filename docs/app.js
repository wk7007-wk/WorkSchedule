...
// Firebase에서도 로드 (비동기)
(async()=>{try{
  const fb=await fbGet(FB_WS+'/fixed_schedules');
...
async function purgeEmployeeReferences(empId){
  const stats = {schedules:0, dayoffs:0, statuses:0, localCaches:0, failed:false};
  ...
  try{
    ...
    if(allSchedules && typeof allSchedules === 'object'){
      const tasks = [];
      Object.keys(allSchedules).forEach(dk => {
        if(dk < todayKey) return;
        const day = allSchedules[dk];
        if(day && typeof day === 'object' && Object.prototype.hasOwnProperty.call(day, empId)){
          stats.schedules++;
          tasks.push(fbDelete(FB_SCHEDULES+'/'+dk+'/'+empId));
        }
        if(weekSchedules[dk] && Object.prototype.hasOwnProperty.call(weekSchedules[dk], empId)){
          delete weekSchedules[dk][empId];
        }
        const cached = lsLoadSchedule(dk);
        if(cached && Object.prototype.hasOwnProperty.call(cached, empId)){
          delete cached[empId];
          lsSaveSchedule(dk, cached);
          stats.localCaches++;
        }
      });
      if(Object.prototype.hasOwnProperty.call(daySchedule, empId)){
        delete daySchedule[empId];
        lsSaveSchedule(dateKey(currentDate), daySchedule);
      }
      const results = await Promise.all(tasks);
      if(results.some(ok => !ok)) stats.failed = true;
    }
  }catch(e){
    console.error('purgeEmployeeReferences schedules error', e);
    stats.failed = true;
  }

  try{
    if(dayoffs[empId] && typeof dayoffs[empId] === 'object'){
      Object.keys(dayoffs[empId]).forEach(dk => {
        if(dk >= todayKey){
          delete dayoffs[empId][dk];
          stats.dayoffs++;
        }
      });
      if(Object.keys(dayoffs[empId]).length === 0) delete dayoffs[empId];
      lsSaveDayoffs();
      const ok = stats.dayoffs > 0
        ? await fbPut(FB_DAYOFFS+'/'+empId, dayoffs[empId] || null)
        : true;
      if(!ok) stats.failed = true;
    }
  }catch(e){
    console.error('purgeEmployeeReferences dayoffs error', e);
    stats.failed = true;
  }

  try{
    const allStatuses = await fbGet(FB_WS+'/shift_status');
    if(allStatuses && typeof allStatuses === 'object'){
      const tasks = [];
      Object.keys(allStatuses).forEach(dk => {
        if(dk < todayKey) return;
        const row = allStatuses[dk];
        if(row && typeof row === 'object' && Object.prototype.hasOwnProperty.call(row, empId)){
          stats.statuses++;
          tasks.push(fbDelete(FB_WS+'/shift_status/'+dk+'/'+empId));
        }
        delete shiftStatus[dk+'_'+empId];
      });
      lsSaveShiftStatus();
      const results = await Promise.all(tasks);
      if(results.some(ok => !ok)) stats.failed = true;
    ...
    console.error('purgeEmployeeReferences shift_status error', e);
    stats.failed = true;
  }

  try{
    delete dayAttendance[empId];
    localStorage.removeItem('ws_shared_'+empId);
  }catch(e){}
...
function performRenderAll(){
  ...
  if(isSectionOpen('weekBody')){
    try { renderWeek(); } catch(e) { console.error('[WS] renderWeek error:', e); }
  }
  if(isSectionOpen('shareBody')){
    try { buildCalButtons(); } catch(e) { console.error('[WS] buildCalButtons error:', e); }
    try { buildKakaoPerEmp(); } catch(e) { console.error('[WS] buildKakaoPerEmp error:', e); }
  }
}
...
function renderEmpList(){
  ...
  $list.querySelectorAll('[data-del]').forEach(btn => {
    btn.addEventListener('click', async()=>{
      const empId = btn.dataset.del;
      const empName = employees[empId]?.name || empId;
      if(!confirm(empName+' 삭제?\n연관 스케줄/휴무/보정상태도 함께 정리됩니다.')) return;
      const cleanup = await purgeEmployeeReferences(empId);
      if(cleanup.failed){
        showToast('연관 데이터 정리 실패');
        return;
      }
      ...
      if(ok){
        ...
        renderEmpList();
        renderAll();
        showToast('삭제됨 · 스케줄 '+cleanup.schedules+'건 정리');
      }
    });
  });
}
...
function setupCollapsible(toggleId, arrowId, bodyId, storageKey, defaultOpen){
  ...
  $(toggleId).addEventListener('click', ()=>{
    ...
    if(open && (bodyId === 'weekBody' || bodyId === 'shareBody')){
      renderAll(true);
    }
    if(open && bodyId === 'infoBody'){

...
assert.equal(normalized.entries.length, 10);
assert.equal(normalized.memos.length, 0);

const categories = new Set(normalized.entries.map((item) => item.category));
for (const cat of ['customer_support', 'platform_help', 'coupon', 'work', 'discount', 'delivery', 'task']) {
...
for (const item of normalized.entries) {
  ...
  assert.match(item.categoryLabel, /고객응대|앱주문|쿠폰|근무|배달|할일|행사/);
  assert.doesNotMatch([item.title, item.summary, item.body].join(' '), /source|status|updated_at|search_text|원문|복붙|DB|database/i);
...
assert.ok(merged.some((item) => item.title === 'BBQ 쿠폰 사용 안내'));

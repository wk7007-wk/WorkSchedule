...
const shiftFixedStart = appSource.indexOf("$('shiftFixed').addEventListener('click'");
const shiftDayoffStart = appSource.indexOf("$('shiftDayoff').addEventListener('click'");
const saveStdStart = appSource.indexOf('async function saveStd(){');
const briefingStart = appSource.indexOf('// === briefing ===');
...
assert.ok(shiftFixedStart > 0 && shiftDayoffStart > shiftFixedStart, 'shiftFixed block missing');
assert.ok(saveStdStart > 0 && briefingStart > saveStdStart, 'saveStd block missing');
const shiftFixedBlock = appSource.slice(shiftFixedStart, shiftDayoffStart);
const saveStdBlock = appSource.slice(saveStdStart, briefingStart);
assert.match(shiftFixedBlock, /commitFixedSchedule\(smE,fixed,\{reload:'full'\}\)/);
assert.doesNotMatch(shiftFixedBlock, /FW\+['"]\/overrides['"]/);
assert.match(saveStdBlock, /PREVIEW_ONLY/);
assert.match(saveStdBlock, /commitDailySchedule\(d,eid,action,shift,\{reload:'week'\}\)/);
assert.match(saveStdBlock, /saveStdRequestPreview\(payload\)/);

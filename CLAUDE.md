...
## 역할 시스템
- **역할**: 주방, 차배달, 오토바이 — 멀티 선택 가능
- **색상 표시**: 주방=#E67E22(주황), 차배달=#4ECDC4(민트), 오토바이=#FFD700(노랑)
- **이모지/그림 아이콘 지양** → 색상 텍스트로 식별
...
## 직원별 능력 (가능 역할)
- **이원규**: 주방, 차배달, 오토바이 (모두 가능)
- **박준모**: 주방, 차배달
- **대유**: 주방, 차배달
- **전묘정, 리, 히오, 권연옥**: 주방만

## 충돌 규칙 (시간대별 인원 제한)
- **차배달**: 동시 최대 1명 (2명 겹칠 시 1명 주방편입 → 빨간음영 표시)
- **오토바이**: 동시 최대 1명 (겹칠 시 주방편입)
- **주방**: 피크(17~20시) 최대 4명, 그 외 최대 3명
- **주방 최소**: 17:00~03:00 최소 2명 유지, 그 외 최소 1명
- **충돌 표시**: 초과 직원 시프트 블록에 빨간 음영 + 경고 메시지
- **인원 체크 색상**: 초록=충족, 빨강=초과, 주황=부족

## 배달대행 (가상 멤버)
- **영업시간**: 10:00 ~ 01:00 (새벽 1시)
- **역할**: 배달자(차+오토바이) 공백 시간대에 자동으로 배달 파트 충족
...
- **주방 최소**: 17:00~03:00 = 2명, 그 외 = 1명
- **주방 최대**: 피크(17~20시) = 4명, 그 외 = 3명
- **차배달 최대**: 동시 1명
- **오토바이 최대**: 동시 1명
- 물류: 16:30 이후 인원 체크
- **충족 표시**: 초록(충족), 빨강(초과), 주황(부족)
